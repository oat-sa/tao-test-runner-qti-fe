 ...
 */
define(['taoQtiTest/runner/plugins/tools/highlighter/collection'], function(highlighterCollection) {
    'use strict';
    ...
    QUnit.cases.init(api).test('plugin API ', function(data, assert) {
        var coll = highlighterCollection();
        assert.expect(1);

        assert.equal(
            typeof coll[data.name],
            'function',
            `The highlighterCollection instances expose a "${  data.name  }" function`
        );
    });
