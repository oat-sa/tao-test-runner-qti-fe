...
define(['jquery', 'taoQtiTest/runner/plugins/controls/timer/component/timerbox'], function($, timerboxFactory) {
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.notStrictEqual(
            timerboxFactory(),
            timerboxFactory(),
            'The timerboxFactory factory provides a different object on each call'
        );
    });
    ...
    QUnit.cases
        ...
        .test('Component API ', function(data, assert) {
            ...
            assert.equal(
                ...
                'function',
                `The timerboxFactory exposes the component method "${  data.title}`
            );
        });
    ...
    QUnit.cases
        ...
        .test('Eventifier API ', function(data, assert) {
            ...
            assert.equal(
                typeof instance[data.title],
                'function',
                `The timerboxFactory exposes the eventifier method "${  data.title}`
            );
        });
    ...
    QUnit.cases
        ...
        .test('Instance API ', function(data, assert) {
            ...
            assert.equal(
                typeof instance[data.title],
                'function',
                `The timerboxFactory exposes the method "${  data.title}`
            );
        });
        ...
        timerboxFactory({
            ...
            .on('update.first', function() {
                ...
                this.on('update.second', function() {
                    assert.equal($('.countdown', $element).length, 2, '2 timers are rendered');
                    assert.equal(
                        $('.countdown[data-control="timer-1"]', $element).length,
                        0,
                        'The 1st timer has been removed'
                    );
                    assert.equal(
                        $('.countdown[data-control="timer-2"]', $element).length,
                        1,
                        'The 2nd timer keeps in place'
                    );
