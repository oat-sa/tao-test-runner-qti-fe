...
define([
    ...
    QUnit.cases.init(testCases).test('render feedbacks as alertMessage', function(testCase, assert) {
        var ready = assert.async();
        var renderer;
        ...
        new QtiLoader().loadItemData(itemData, function(_item) {
            ...
            renderer.load(function() {
                ...
                testRunner.on('plugin-render.QtiModalFeedback', function(feedback) {
                    var feedbacks;
                    var $modalsBlock = $('#modalFeedbacks', $result);
                    var countFeedbacks =
                        testCase.feedbacks.choice.length +
                        testCase.feedbacks.inline.length +
                        testCase.feedbacks.order.length;
                    assert.equal(feedback.getState('ready'), true, 'The feedback is rendered');
                    assert.equal(
                        $('.qti-modalFeedback', $modalsBlock).length,
                        countFeedbacks,
                        'modal feedbacks in the special dom element'
                    );
                        ...
                        if ($feedback.length) {
                            if (fb.style) {
                                ...
                                if ($feedback[0].hasAttribute('class')) {
                                    assert.equal(
                                        $feedback.attr('class').trim(),
                                        'modal qti-modalFeedback',
                                        'the unique css class must be qti-modalFeedback'
                                    );
                                } else {
