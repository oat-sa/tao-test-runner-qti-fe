 ...
 */
define(['jquery', 'lodash', 'taoQtiTest/runner/plugins/tools/lineReader/compoundMask'], function(
    $,
    _,
    compoundMaskFactory
) {
    ...
    QUnit.test('setTransforms / applyTransforms', function(assert) {
        ...
        assert.deepEqual(
            allParts.n.overlay.getSize(),
            { width: 500, height: 50 },
            'north overlay has the right dimensions'
        );
        assert.deepEqual(allParts.n.overlay.getPosition(), { x: 50, y: 50 }, 'north overlay has the right position');

        assert.deepEqual(
            allParts.ne.mask.getSize(),
            { width: 100, height: 50 },
            'north-east mask has the right dimensions'
        );
        ...
        assert.deepEqual(
            allParts.se.mask.getSize(),
            { width: 100, height: 200 },
            'south-east mask has the right dimensions'
        );
        ...
        assert.deepEqual(
            allParts.sw.mask.getSize(),
            { width: 50, height: 200 },
            'south-west mask has the right dimensions'
        );
        ...
        assert.deepEqual(
            allParts.nw.mask.getSize(),
            { width: 50, height: 50 },
            'north-west mask has the right dimensions'
        );
    ...
    QUnit.cases
        ...
        .test('Resize', function(data, assert) {
            ...
            mask.init().render($container);

            allParts[data.maskId].mask.resizeTo(data.width, data.height, data.fromLeft || false, data.fromTop || false);
            ...
            assert.deepEqual(
                mask.getDimensions(),
                _.assign({}, dimensions, data.dimensions),
                'dimensions after resize are correct'
            );
            assert.deepEqual(
                mask.getPosition(),
                _.assign({}, position, data.position),
                'positions after resize are correct'
            );
        });
