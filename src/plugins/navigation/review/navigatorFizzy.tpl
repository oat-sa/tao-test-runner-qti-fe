<div class="qti-panel qti-navigator{{#if hidden}} hidden{{/if}} qti-navigator-fizzy">
    <div class="qti-navigator-header" title="{{__ 'Test overview'}}">
        <div class="qti-navigator-text">{{__ 'Test overview'}}</div>
        <a class="icon-close" href="#" onclick="return false" aria-hidden="true"></a>
    </div>
    <nav class="qti-navigator-tree"></nav>
    <div id="qti-navigator-linear" class="qti-navigator-linear">
        <p class="qti-navigator-message">
            {{__ 'In this part of the test navigation is not allowed.'}}
        </p>
    </div>
</div>
