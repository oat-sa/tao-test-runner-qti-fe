...
define([
...
], function(offlineJumpTableFactory, itemStoreFactory, responseStoreFactory, itemsJson, testMapJson) {
    'use strict';

    var offlineJumpTable, responseStore, itemStore;
    ...
    QUnit.cases
        ...
        .test(
            'it jumps to the next item with jumpToNextItem(), considering the branching rules, if there is any',
    ...
    QUnit.test('it jump to the first item of the next section with jumpToNextSection()', function(assert) {
        ...
        Promise.all([offlineJumpTable.addJump('P01', 'S01', 'I01'), offlineJumpTable.jumpToNextSection()]).then(
            function() {
                assert.expect(3);
