 ...
 */
define(['jquery', 'taoQtiTest/runner/plugins/controls/progressbar/renderer/position'], function(
    $,
    ...
    QUnit.test('module', function(assert) {
        ...
        assert.equal(
            typeof positionRendererFactory,
            'function',
            'The positionRendererFactory module exposes a function'
        );
        assert.equal(
            typeof positionRendererFactory(),
            'object',
            'The positionRendererFactory factory produces an object'
        );
        assert.notStrictEqual(
            positionRendererFactory(),
            positionRendererFactory(),
            'The positionRendererFactory factory provides a different object on each call'
        );
    });
    ...
    QUnit.test('Rendering with label', function(assert) {
        ...
        assert.equal(
            $('[data-control="progress-label"]', $container).length,
            0,
            'The component label does not exists yet'
        );
        ...
        positionRendererFactory({}, data)
            .on('update', function() {
                assert.equal(
                    $('[data-control="progress-label"]', $container).length,
                    1,
                    'The component label has been inserted'
                );
                assert.equal(
                    $('[data-control="progress-bar"]', $container).length,
                    1,
                    'The component bar has been inserted'
                );

                assert.equal(
                    $('[data-control="progress-label"]', $container)
                        .text()
                        .trim(),
                    data.label,
                    'The component label is correct'
                );
                assert.equal(
                    $('[data-control="progress-bar"] .progressbar-point', $container).length,
                    data.total,
                    'The component bar has the expected number of points'
                );
                assert.equal(
                    $('[data-control="progress-bar"] .progressbar-point.reached', $container).length,
                    data.position,
                    'The component bar displays the correct position'
                );
                assert.equal(
                    $(
                    ...
                    true,
                    'The component bar displays the current position'
                );

                assert.equal(
                    $('[data-control="progress-label"]:visible', $container).length,
                    1,
