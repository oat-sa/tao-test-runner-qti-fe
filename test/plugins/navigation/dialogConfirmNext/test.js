 ...
 */
define(['jquery', 'lodash', 'taoQtiTest/runner/plugins/navigation/next/dialogConfirmNext'], function(
    $,
    _,
    dialogConfirmNext
) {
    ...
    QUnit.cases.init(dialogApi).test('instance API ', function(data, assert) {
        ...
        assert.equal(
            ...
            'function',
            `The dialogConfirmNext instance exposes a "${data.name}" function`
        );
        instance.destroy();
    });
    ...
    QUnit.test('events', function(assert) {
        ...
        assert.equal(
            modal
                ...
                .text(),
            heading,
            'The dialog box displays the heading'
        );
        assert.equal(
            modal
                .getDom()
                ...
                .text(),
            message,
            'The dialog box displays the message'
        );
    ...
    QUnit.cases.init(['checked', 'unchecked']).test('checkbox', function(data, assert) {
        var ready = assert.async();
        var accept = function() {
