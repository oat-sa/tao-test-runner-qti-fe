...
define([
...
], function(_, mapHelper, mapSample, lightMapSample, activeMapSample) {
    'use strict';
    ...
    QUnit.test('helpers/map.getJump', function(assert) {
        var map = {
            jumps: [{ identifier: 'item-1' }, { identifier: 'item-2' }]
        };
    ...
    QUnit.test('helpers/map.getPart', function(assert) {
        ...
        assert.equal(
            mapHelper.getPart(mapSample, 'testPart-2'),
            mapSample.parts['testPart-2'],
            'The map helper getPart provides the right part'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getSection', function(assert) {
        ...
        assert.equal(
            mapHelper.getSection(mapSample, 'assessmentSection-3'),
            mapSample.parts['testPart-2'].sections['assessmentSection-3'],
            'The map helper getSection provides the right section'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getSections', function(assert) {
        assert.expect(3);
        assert.deepEqual(
            mapHelper.getSections(mapSample),
            _.extend({}, mapSample.parts['testPart-1'].sections, mapSample.parts['testPart-2'].sections),
            'The map helper getSection provides the right sections'
        );
        assert.deepEqual(
            mapHelper.getSections({}),
            {},
            'The map helper getSection does not provide any section when the map is wrong'
        );
        assert.deepEqual(
    ...
    QUnit.test('helpers/map.getActiveItem', function(assert) {
        assert.expect(3);
        assert.deepEqual(
            mapHelper.getActiveItem(activeMapSample),
            _.extend({}, activeMapSample.parts['testPart-1'].sections['assessmentSection-2'].items['item-4']),
            'The map helper getSection provide the active item from map'
        );
        assert.deepEqual(
            mapHelper.getActiveItem(mapSample),
            {},
            'The map helper getActiveItem does not provide any item when there is no active items'
        );
        assert.deepEqual(
            mapHelper.getActiveItem(),
            {},
            'The map helper getActiveItem does not provide any item when the map does not exist'
        );
    });
    ...
    QUnit.test('helpers/map.getNextSections', function(assert) {
        assert.expect(4);
        assert.deepEqual(
            mapHelper.getNextSections(mapSample, 'assessmentSection-3'),
            _.omit(mapSample.parts['testPart-2'].sections, 'assessmentSection-3'),
            'The map helper getSection provides the right sections'
        );
        assert.deepEqual(
            mapHelper.getNextSections(mapSample, 'foo'),
            {},
            'The map helper getSection does not provide any section when the section does not exist'
        );
        assert.deepEqual(
            mapHelper.getNextSections({}, 'foo'),
            {},
            'The map helper getSection does not provide any section when the map is wrong'
        );
        assert.deepEqual(
            mapHelper.getNextSections(),
            {},
            'The map helper getSection does not provide any section when the map does not exist'
        );
    });
    ...
    QUnit.test('helpers/map.getItem', function(assert) {
        ...
        assert.equal(
            mapHelper.getItem(mapSample, 'item-5'),
            mapSample.parts['testPart-1'].sections['assessmentSection-2'].items['item-5'],
            'The map helper getItem provides the right item'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getPartStats', function(assert) {
        ...
        assert.equal(
            mapHelper.getPartStats(mapSample, 'testPart-2'),
            mapSample.parts['testPart-2'].stats,
            'The map helper getPartStats provides the right stats'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getSectionStats', function(assert) {
        ...
        assert.equal(
            mapHelper.getSectionStats(mapSample, 'assessmentSection-3'),
            mapSample.parts['testPart-2'].sections['assessmentSection-3'].stats,
            'The map helper getSectionStats provides the right stats'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getScopeStats', function(assert) {
        ...
        assert.equal(
            ...
            mapHelper.getScopeStats(mapSample, 6, 'test'),
            'The map helper getScopeStats use the "test" scope by default'
        );
        assert.equal(
            mapHelper.getScopeStats(mapSample, 6, 'test'),
            mapSample.stats,
            'The map helper getScopeStats provides the right stats when the scope is "test"'
        );
        assert.equal(
            mapHelper.getScopeStats(mapSample, 6, 'part'),
            mapSample.parts['testPart-2'].stats,
            'The map helper getScopeStats provides the right stats when the scope is "part"'
        );
        assert.equal(
            mapHelper.getScopeStats(mapSample, 6, 'section'),
            mapSample.parts['testPart-2'].sections['assessmentSection-3'].stats,
            'The map helper getScopeStats provides the right stats when the scope is "section'
        );
        assert.equal(
            mapHelper.getScopeStats(mapSample, 100, 'test'),
            mapSample.stats,
            'The map helper getScopeStats still provide any stats when the position does not exist but the scope is "test"'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getScopeMap', function(assert) {
        ...
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 6),
            mapHelper.getScopeMap(mapSample, 6, 'test'),
            'The map helper getScopeMap use the "test" scope by default'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 6, 'test'),
            expectedTestMap,
            'The map helper getScopeMap provides the right content when the scope is "test"'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 6, 'part'),
            expectedPartMap,
            'The map helper getScopeMap provides the right content when the scope is "part"'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 6, 'section'),
            expectedSectionMap,
            'The map helper getScopeMap provides the right content when the scope is "section'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 100, 'test'),
            expectedTestMap,
            'The map helper getScopeMap still provide any map when the position does not exist but the scope is "test"'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 100, 'part'),
            expectedNotFound,
            'The map helper getScopeMap does not provide any map when the position does not exist and the scope is "part"'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(mapSample, 100, 'section'),
            expectedNotFound,
            'The map helper getScopeMap does not provide any map when the section does not exist and the scope is "section"'
        );
        assert.deepEqual(
            mapHelper.getScopeMap({}, 1),
            expectedEmpty,
            'The map helper getScopeMap does not provide any map when the map is wrong'
        );
        assert.deepEqual(
            mapHelper.getScopeMap(),
            expectedEmpty,
            'The map helper getScopeMap does not provide any map when the map does not exist'
        );
    });
    ...
    QUnit.test('helpers/map.getItemPart', function(assert) {
        ...
        assert.equal(
            mapHelper.getItemPart(mapSample, 8),
            mapSample.parts['testPart-2'],
            'The map helper getItemPart provides the right part'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getItemSection', function(assert) {
        ...
        assert.equal(
            mapHelper.getItemSection(mapSample, 8),
            mapSample.parts['testPart-2'].sections['assessmentSection-4'],
            'The map helper getItemSection provides the right section'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getItemAt', function(assert) {
        ...
        assert.equal(
            mapHelper.getItemAt(mapSample, 8),
            mapSample.parts['testPart-2'].sections['assessmentSection-4'].items['item-9'],
            'The map helper getItemAt provides the right item'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.getItemIdentifier', function(assert) {
        ...
        assert.equal(
            mapHelper.getItemIdentifier(mapSample, 8),
            'item-9',
            'The map helper getItemIdentifier provides the right identifier'
        );
        assert.equal(
            mapHelper.getItemIdentifier(mapSample, 'item-8'),
            'item-8',
            'The map helper getItemIdentifier provides the right identifier if directly provided'
        );
        assert.equal(
    ...
    QUnit.test('helpers/map.each', function(assert) {
        ...
        result = mapHelper.each(mapSample, function(item, section, part, map) {
            assert.equal(
                item,
                mapHelper.getItemAt(mapSample, item.position),
                'The right item is provided to each callback'
            );
            assert.equal(
                section,
                mapHelper.getItemSection(mapSample, item.position),
                'The right section is provided to each callback'
            );
            assert.equal(
                part,
                mapHelper.getItemPart(mapSample, item.position),
                'The right part is provided to each callback'
            );
        ...
        assert.equal(
            mapHelper.each(mapSample),
            mapSample,
            'The map helper each returns the provided map even if no callback has been provided'
        );
    });
    ...
    QUnit.test('helpers/map.updateItemStats', function(assert) {
        ...
        assert.equal(
            item.answered,
            true,
            'The item is still answered, but it should be ignored since it is informational'
        );
